...
```{r}
ice_cover <- ice_cover %>% filter(year4 >= "1970") %>% 
   mutate(ice_off = as.Date(ice_off))

ice_cover %>%  mutate(ice_off_yday = lubridate::yday(ice_off)) %>% na.omit() %>% ggplot(aes(
  x = ice_off,
 # x = factor(ice_off_yday),
  y = ice_duration)) + geom_point(col = "darkred") + labs(x = "Ice off", y = "Number of days of lasting ice (ice duration)", title = "Ice duration based on date of thaw") + theme_minimal()  
